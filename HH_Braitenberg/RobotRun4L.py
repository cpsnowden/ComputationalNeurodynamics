...
Ns = 2  # Sensor neurons. Try 1, 4, and 8
Nm = 2  # Motor neurons. Try 1, 4, and 8
Ni = 2
...
for lr in xrange(net.Nlayers):
  net.layer[lr].v = v0 * np.ones(net.layer[lr].N)
  net.layer[lr].m = m0 * np.ones(net.layer[lr].N)
  net.layer[lr].n = n0 * np.ones(net.layer[lr].N)
  net.layer[lr].h = h0 * np.ones(net.layer[lr].N)
  net.layer[lr].firings = np.array([])

# Simulation parameters
Tmax = 10000  # Simulation time in milliseconds
dt   = 1000    # Robot step size in milliseconds
...
for lr in xrange(net.Nlayers):
  ...
  I[lr] = np.zeros([dt, net.layer[lr].N])
...
for t in xrange(len(T)):
  ...
  for t2 in xrange(dt):
    ...
    net.layer[2].I = rn.randn(N2)
    net.layer[3].I = rn.randn(N3)

    if(Ni != None):
      #Noisy base current for Inhibitory Neurons
      net.layer[4].I = rn.randn(N3)
      net.layer[5].I = rn.randn(N3)
  ...
  plt.subplot(325)
  plt.plot(v[4])
  plt.title('Left inhib neurons')
  plt.ylabel('Membrane potential (mV)')
  plt.ylim(-90, 60)

  plt.subplot(326)
  plt.plot(v[5])
  plt.title('Right inhib neurons')
  plt.ylim(-90, 60)
